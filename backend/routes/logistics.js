...
router.get('/logistics/trucking', async (_req, res) => {
  try {
    ...
    res.json(getAll(db, 'SELECT * FROM trucking_quotations ORDER BY date_of_activity DESC, id DESC'));
...
router.post('/logistics/trucking', upload.single('file'), async (req, res) => {
  try {
    ...
    res.status(201).json(getOne(db, 'SELECT * FROM trucking_quotations WHERE id=?', [id]));
  } catch (err) {
    if (err.message.includes('UNIQUE')) return res.status(409).json({ error: 'Quote number already exists' });
...
router.put('/logistics/trucking/:id', upload.single('file'), async (req, res) => {
  try {
    const { db, save } = await dbPromise;
    const existing = getOne(db, 'SELECT * FROM trucking_quotations WHERE id=?', [+req.params.id]);
    ...
    db.run(
      `UPDATE trucking_quotations SET quote_number=?, trucking_service=?, date_of_activity=?, sites=?,
       total_amount=?, status=?, file_name=?, file_path=?, updated_at=datetime('now') WHERE id=?`,
    ...
    save();
    res.json(getOne(db, 'SELECT * FROM trucking_quotations WHERE id=?', [+req.params.id]));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete('/logistics/trucking/:id', async (req, res) => {
  try {
    ...
    db.run('DELETE FROM trucking_quotations WHERE id=?', [+req.params.id]);
    save();
...
router.get('/logistics/manpower', async (_req, res) => {
  try {
    const { db } = await dbPromise;
    res.json(getAll(db, 'SELECT * FROM manpower_requests ORDER BY created_at DESC'));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get('/logistics/manpower/next-request-no', async (_req, res) => {
  try {
    ...
    const result = db.exec("SELECT COUNT(*) FROM manpower_requests WHERE request_no LIKE 'MP-" + today + "-%'");
    const count = result[0] ? Number(result[0].values[0][0]) : 0;
...
router.post('/logistics/manpower', upload.single('file'), async (req, res) => {
  try {
    ...
    const today = new Date().toISOString().slice(0,10).replace(/-/g,'');
    const existing_count = db.exec(
      "SELECT COUNT(*) FROM manpower_requests WHERE request_no LIKE 'MP-" + today + "-%'"
    );
    const seq = (existing_count[0] ? Number(existing_count[0].values[0][0]) : 0) + 1;
    ...
    const id = lastId(db); save();
    res.status(201).json(getOne(db, 'SELECT * FROM manpower_requests WHERE id=?', [id]));
  } catch (err) {
    if (err.message.includes('UNIQUE')) return res.status(409).json({ error: 'Request number already exists' });
    res.status(500).json({ error: err.message });
  }
});
...
router.put('/logistics/manpower/:id', upload.single('file'), async (req, res) => {
  try {
    const { db, save } = await dbPromise;
    const existing = getOne(db, 'SELECT * FROM manpower_requests WHERE id=?', [+req.params.id]);
    if (!existing) return res.status(404).json({ error: 'Not found' });
    ...
    db.run(
      `UPDATE manpower_requests SET location=?, machine_type=?, machine_quantity=?, manpower_quantity=?,
       purpose=?, unit_price=?, remarks=?, total=?, file_name=?, file_path=?, updated_at=datetime('now') WHERE id=?`,
    ...
    save();
    res.json(getOne(db, 'SELECT * FROM manpower_requests WHERE id=?', [+req.params.id]));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete('/logistics/manpower/:id', async (req, res) => {
  try {
    ...
    db.run('DELETE FROM manpower_requests WHERE id=?', [+req.params.id]);
    save();
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get('/logistics/sites-activity', async (_req, res) => {
  try {
    const { db } = await dbPromise;
    res.json(getAll(db, 'SELECT * FROM sites_activity ORDER BY activity_date DESC, id DESC'));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/logistics/sites-activity', upload.single('file'), async (req, res) => {
  try {
    ...
    const id = lastId(db); save();
    res.status(201).json(getOne(db, 'SELECT * FROM sites_activity WHERE id=?', [id]));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.put('/logistics/sites-activity/:id', upload.single('file'), async (req, res) => {
  try {
    const { db, save } = await dbPromise;
    const existing = getOne(db, 'SELECT * FROM sites_activity WHERE id=?', [+req.params.id]);
    if (!existing) return res.status(404).json({ error: 'Not found' });
    const { site_name, activity_type, activity_date, location, description, status } = req.body;
    const file_name = req.file ? req.file.filename : existing.file_name;
    const file_path = req.file ? '/uploads/logistics/' + req.file.filename : existing.file_path;
    ...
    db.run(
      `UPDATE sites_activity SET site_name=?, activity_type=?, activity_date=?, location=?,
       description=?, status=?, file_name=?, file_path=?, updated_at=datetime('now') WHERE id=?`,
    ...
    save();
    res.json(getOne(db, 'SELECT * FROM sites_activity WHERE id=?', [+req.params.id]));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete('/logistics/sites-activity/:id', async (req, res) => {
  try {
    ...
    db.run('DELETE FROM sites_activity WHERE id=?', [+req.params.id]);
    save();
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.get('/logistics/waybills', async (_req, res) => {
  try {
    const { db } = await dbPromise;
    res.json(getAll(db, 'SELECT * FROM waybills ORDER BY date DESC, id DESC'));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/logistics/waybills', upload.single('file'), async (req, res) => {
  try {
    ...
    const id = lastId(db); save();
    res.status(201).json(getOne(db, 'SELECT * FROM waybills WHERE id=?', [id]));
  } catch (err) {
    if (err.message.includes('UNIQUE')) return res.status(409).json({ error: 'Waybill number already exists' });
    res.status(500).json({ error: err.message });
  }
});
...
router.put('/logistics/waybills/:id', upload.single('file'), async (req, res) => {
  try {
    const { db, save } = await dbPromise;
    const existing = getOne(db, 'SELECT * FROM waybills WHERE id=?', [+req.params.id]);
    if (!existing) return res.status(404).json({ error: 'Not found' });
    const { waybill_number, date, origin, destination, notes } = req.body;
    const file_name = req.file ? req.file.filename : existing.file_name;
    const file_path = req.file ? '/uploads/logistics/' + req.file.filename : existing.file_path;
    ...
    db.run(
      `UPDATE waybills SET waybill_number=?, date=?, origin=?, destination=?, notes=?,
       file_name=?, file_path=?, updated_at=datetime('now') WHERE id=?`,
    ...
    save();
    res.json(getOne(db, 'SELECT * FROM waybills WHERE id=?', [+req.params.id]));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.delete('/logistics/waybills/:id', async (req, res) => {
  try {
    ...
    db.run('DELETE FROM waybills WHERE id=?', [+req.params.id]); save();
    res.status(204).end();
  } catch (err) { res.status(500).json({ error: err.message }); }
});
