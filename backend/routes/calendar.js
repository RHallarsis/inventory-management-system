...
router.get('/calendar/tasks', async (req, res) => {
  try {
    ...
    let tasks = await db.getAll(
      'SELECT id,task_date,task_time,title,description,category,priority,status,color,created_at,updated_at FROM calendar_tasks ORDER BY task_date ASC, task_time ASC NULLS LAST, created_at ASC'
    );
...
router.get('/calendar/tasks/:id', async (req, res) => {
  try {
    ...
    const task = await db.getOne(
      'SELECT id,task_date,task_time,title,description,category,priority,status,color,created_at,updated_at FROM calendar_tasks WHERE id=?',
      [+req.params.id]
    );
...
router.post('/calendar/tasks', async (req, res) => {
  try {
    const { db } = await dbPromise;
    const {
      task_date, title, task_time = null,
    ...
    const id = await db.insert(
      `INSERT INTO calendar_tasks (task_date,task_time,title,description,category,priority,status,color) VALUES (?,?,?,?,?,?,?,?)`,
      [task_date, task_time||null, title.trim(), description.trim(), category, priority, status, color]
    );

    // Broadcast to all Line friends (fire-and-forget, never blocks response)
    const task = { id, task_date, task_time, title: title.trim(), description: description.trim(), category, priority, status, color };
...
router.put('/calendar/tasks/:id', async (req, res) => {
  try {
    ...
    await db.run(
      `UPDATE calendar_tasks SET task_date=?,task_time=?,title=?,description=?,category=?,priority=?,status=?,color=?,updated_at=NOW() WHERE id=?`,
      [task_date, task_time||null, title.trim(), description.trim(), category, priority, status, color, +req.params.id]
    );

    // Broadcast to all Line friends (fire-and-forget)
    const task = { id: +req.params.id, task_date, task_time, title: title.trim(), description: description.trim(), category, priority, status, color };
